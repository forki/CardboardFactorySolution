...
module Calculation =
    ...
    let private parametersToExpressionParameters (parameters:Map<string, Product.ProductParameter>) = 
        parameters
        |> Map.map (fun key value -> box value.Value)

    let private createAndEvaluateExpresion (parameters:Map<string, Product.ProductParameter>) (formula:string) =
    ...
    let private findFormula (corrugationType:CorrugationTypes.Enum) (formulas:Map<CorrugationTypes.Enum, Product.LengthFormula>)  =
        let formula = 
            formulas
