...
module ProductTypeTest =
    ...
    let private defaultSubProduct = {
        ...
        Product.SubProduct.LengthOneFormulas = Map<CorrugationTypes.Enum, Product.LengthFormula>(seq [])
        Product.SubProduct.LengthTwoFormulas  = Map<CorrugationTypes.Enum, Product.LengthFormula>(seq [])
    }

    let private defaultProductType = {
        Product.ProductType.Name = "Default";
        Product.ProductType.Parameters = 
            Map<string, Product.ProductParameter>(
                seq 
                    [
                        lengthName, {Product.ProductParameter.Name = lengthName; Product.ProductParameter.Value = Nullable<double>()};
                        widthName, {Product.ProductParameter.Name = widthName; Product.ProductParameter.Value = Nullable<double>()} ]
                    );
    ...
    let ``Calculate lengths for Box0200``() =
        ...
        let parameters = 
            Map<string, Product.ProductParameter>(
                seq
                    [
                        ("Длина", { Name = "Длина"; Value = Nullable 0.300});
                        ("Ширина", { Name = "Ширина"; Value = Nullable 0.300});
                        ("Высота", { Name = "Высота"; Value = Nullable 0.200});
                    ])

        let lengthOneFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (executionOne, { CorrugationType = executionOne; FormulaText = "2 * ([Длина] + [Ширина]) + 0.058"});
                        (executionTwo, { CorrugationType = executionTwo; FormulaText = "2 * ([Длина] + [Ширина]) + 0.038"});
                    ])

        let lengthTwoFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (executionOne, { CorrugationType = executionOne; FormulaText = "[Высота] + [Ширина] / 2 + 0.006"});
                        (executionTwo, { CorrugationType = executionTwo; FormulaText = "[Высота] + [Ширина] / 2 + 0.002"});
                    ])
    ...
    let ``Calculate lengths with custom values``(enum:CorrugationTypes.Enum) =
        let parameters = 
            Map<string, Product.ProductParameter>(
                seq
                    [
                        (lengthName, { Name = lengthName; Value = Nullable 120.0});
                        (widthName, { Name = widthName; Value = Nullable 240.0});
                    ])
        ...
        let lengthOneFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (CorrugationTypes.Enum.All, { CorrugationType = CorrugationTypes.Enum.All; FormulaText = sprintf "[%s] + [%s]" lengthName lengthName});
                    ])
        ...
        let lengthTwoFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (CorrugationTypes.Enum.All, { CorrugationType = CorrugationTypes.Enum.All; FormulaText = sprintf "[%s] + [%s] + 50" widthName lengthName});
                    ])

        let sub =  { Product.SubProduct.Name = "Sub"; Product.SubProduct.LengthOneFormulas = lengthOneFormulas; Product.SubProduct.LengthTwoFormulas = lengthTwoFormulas }
        let list = [sub]
        let product = { defaultProductType with Name = "Test"; SubProducts = list; Parameters = parameters }
    ...
    let ``Calculate lengths with not existing argument``() =
        let lengthOneFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (CorrugationTypes.Enum.All, { CorrugationType = CorrugationTypes.Enum.All; FormulaText = "[BadParameter]"});
                    ])
    ...
    let ``Calculate lengths with no formula``() =
        let lengthOneFormulas = Map<CorrugationTypes.Enum, Product.LengthFormula>(seq [])
        let sub =  { defaultSubProduct with Product.SubProduct.LengthOneFormulas = lengthOneFormulas; }
        let list = [sub]
        let product = { defaultProductType with Name = "Test"; SubProducts = list }
    ...
    let ``Calculate lengths with empty formula``() =
        let lengthOneFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (CorrugationTypes.Enum.All, { CorrugationType = CorrugationTypes.Enum.All; FormulaText = String.Empty});
                    ])
        let sub =  { defaultSubProduct with Product.SubProduct.LengthOneFormulas = lengthOneFormulas; }
        let list = [sub]
        let product = { defaultProductType with Name = "Test"; SubProducts = list }
    ...
    let ``Calculate lengths with wrong formula``() =
        let lengthOneFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (CorrugationTypes.Enum.All, { CorrugationType = CorrugationTypes.Enum.All; FormulaText = "=-5224=+-342fff....+342---2342"});
                    ])
        let sub =  { defaultSubProduct with Product.SubProduct.LengthOneFormulas = lengthOneFormulas; }
        let list = [sub]
        let product = { defaultProductType with Name = "Test"; SubProducts = list }
    ...
    let ``Calculate lengths with new values and formulas``(enum:CorrugationTypes.Enum, length:double, width:double, formulaLengthOne:string, formulaLengthTwo:string, result1:double, result2:double) =
        let parameters = 
            Map<string, Product.ProductParameter>(
                seq
                    [
                        (lengthName, { Name = lengthName; Value = Nullable length});
                        (widthName, { Name = widthName; Value = Nullable width});
                    ])
        ...
        let lengthOneFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (CorrugationTypes.Enum.All, { CorrugationType = CorrugationTypes.Enum.All; FormulaText = formulaLengthOne});
                    ])
        ...
        let lengthTwoFormulas = 
            Map<CorrugationTypes.Enum, Product.LengthFormula>(
                seq 
                    [
                        (CorrugationTypes.Enum.All, { CorrugationType = CorrugationTypes.Enum.All; FormulaText = formulaLengthTwo});
                    ])
