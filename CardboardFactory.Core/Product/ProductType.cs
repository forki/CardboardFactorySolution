...
//        public Dictionary<string, Domain.Product.Product.ProductParameter> Parameters { get; }
//        public List<Domain.Product.Product.SubProduct> SubProducts { get; }
//        public string StampKnivesLengthFormula { get; set; }

//        public ProductType() : this((string)null) { }

//        public ProductType(string name) {
//            Parameters = new Dictionary<string, Domain.Product.Product.ProductParameter>();
//            SubProducts = new List<Domain.Product.Product.SubProduct>();
//            Name = name ?? "DEFAULT";
//        }

//        public ProductType(ProductType other) {
//            Parameters = new Dictionary<string, Domain.Product.Product.ProductParameter>(other.Parameters.Values
//                .ToDictionary(parameter => parameter.Name, parameter => new Domain.Product.Product.ProductParameter (parameter.Name, parameter.Value)));
//            SubProducts = new List<Domain.Product.Product.SubProduct>(other.SubProducts.Select(product => new Domain.Product.Product.SubProduct(product)));
//            Name = other.Name;
...
//            foreach (Domain.Product.Product.SubProduct subProduct in SubProducts) {
...
//            foreach (KeyValuePair<string, Domain.Product.Product.ProductParameter> parameter in Parameters) {
...
//        private double CalculateSheetLength(CorrugationTypes.Enum corrugationType, Domain.Product.Product.SubProduct subProduct) {
//            Domain.Product.Product.LengthFormula formula = subProduct.LengthOneFormulas.Where(pair => pair.Key.HasFlag(corrugationType)).Select(pair => pair.Value).FirstOrDefault();
...
//        private double CalculateSheetWidth(CorrugationTypes.Enum corrugationType, Domain.Product.Product.SubProduct subProduct) {
//            Domain.Product.Product.LengthFormula formula = subProduct.LengthTwoFormulas.Where(pair => pair.Key.HasFlag(corrugationType)).Select(pair => pair.Value).FirstOrDefault();
//            return CreateAndEvaluateExpresion(formula?.FormulaText ?? throw new ProductTypeNoFormulaException($"No formula found for {corrugationType}"));
//        }
...
//                return;
//            }
//            if (Parameters.TryGetValue(name, out Domain.Product.Product.ProductParameter parameter)) {
...
//        private double GetLangthOneParameter(string name, string pattern, Func<CorrugationTypes.Enum, Domain.Product.Product.SubProduct, double> func) {
//            if (name.Equals(pattern)) {
//                Domain.Product.Product.SubProduct subProduct = SubProducts.SingleOrDefault();
